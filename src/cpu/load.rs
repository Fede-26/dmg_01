...
use super::instruction::{Instruction, LoadType, LoadByteSource, LoadByteTarget, LoadWordTarget, Indirect};
use crate::cpu::CPU;

pub fn execute(cpu: &mut CPU, instruction: Instruction) -> (u16, u8) {
    let load_type;
    if let Instruction::LD(lt) = instruction {
        load_type = lt; } else {panic!();}
            ...
            LoadType::AFromIndirect(source) => {
                cpu.registers.a = match source {
                    ...
                    Indirect::LastByteIndirect => {
                        cpu.bus.read_byte(0xFF00 + cpu.registers.c as u16)
                    }
                };
